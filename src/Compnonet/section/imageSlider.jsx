    ...
    return (
       <div className="w-[100%] h-[600px] bg-white flex items-center justify-center relative">
            <div
                ...
                onMouseLeave={() => {if(state.isDrag) {dispatch({type: 'handlerMouseUp', payload: {slider: imgSlider.current} })}}}

                onTouchStart={(event)=> {dispatch({type: 'handlerMouseDown', payload: {client:  event.touches[0].clientX, slider: imgSlider.current}})}}
                onTouchMove={(event) => {dispatch({type: 'handlerMouseMove', payload: {client:  event.touches[0].clientX, slider: imgSlider.current}})}}
