...
const EditFooterMenu = () => {
    ...
    const getRequestMenu =  async (id) => {
        try{
            await api.get(`tables/footer/menuFooter/menu.php/${id}` ,{withCredentials: true}).then((res) => {
                dispatch({type: 'GetRequest', payload : res.data.title})
            })
        }
        catch(err){
            console.error('message: ', err);
            navigate('/panelAdmin/footer/menuWhiteBoard')
    ...
    const editItems = async (event, id) => {
        ...
        try{
            await api.put(`tables/footer/menuFooter/edit.php/${id}`, state).then((res) => {
                res.data;
                navigate('/panelAdmin/footer/menuWhiteBoard')
            })
        }
