...
const EditSessionPapular = () => {
    ...
    const getRequestPapular =  async (id) => {
        try{
            await api.get(`tables/session/papular/papular.php/${id}` ,{withCredentials: true}).then((res) => {
    ...
    const editItems = async (event, id) => {
        ...
        try{
            await api.put(`tables/session/papular/edit.php/${id}`, state).then((res) => {
                res.data;
                navigate('/panelAdmin/session/Popular')
            })
        }
...
export default EditSessionPapular;