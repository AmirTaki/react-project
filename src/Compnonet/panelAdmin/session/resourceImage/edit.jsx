...
const EditSessionResourceImage = () => {
    const {id} = useParams()
    const navigate =  useNavigate()

    const reducer = (state, action) => {
        switch(action.type){
            case "GET_REQUST_RESOURCE":
   ...
   const getResourceImageRequest = async (id) => {
        try{
            await api.get(`tables/session/resourceImage/resource.php/${id}`, {withCredentials: true}).then((res) => {
                dispatch({type: 'GET_REQUST_RESOURCE', payload: res.data})
            })
        }
        catch(err){
    ...
    useEffect(() => {getResourceImageRequest(id)}, [])
    ...
    const editResourceItem = async (event) => {
        ...
        try{
            await api.post(`tables/session/resourceImage/edit.php`, formData,{
                headers : {
                    'Content-Type': 'multipart/form-data',
               }
            }).then((res) => {
                res.data;
                navigate('/panelAdmin/session/resourceImage')  
            })
        }
        catch(err){
                ...
                <div className="flex flex-col justify-center items-center">
                    ...
                    <form>
                        ...
                        <div className="flex justify-center items-center">
                            <input 
                                onClick={(event) => {editResourceItem(event)}}
...
export default EditSessionResourceImage;