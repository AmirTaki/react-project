...
const EditSessionMenu = () => {
    ...
    const getRequestMenu =  async (id) => {
        try{
            await api.get(`tables/session/sessionMenu/menu.php/${id}` ,{withCredentials: true}).then((res) => {
    ...
    const editItems = async (event, id) => {
        ...
        try{
            await api.put(`tables/session/sessionMenu/edit.php/${id}`, state).then((res) => {
                res.data;
                navigate('/panelAdmin/session/menuSession')
            })
        }
        catch(err) {
